...
import { isValidEmail } from "@/utils/validation-investor-profile-creation";
  ...
  const handleChange = (e) => {
    ...
    if (name === "professionalEmail") {
      if (!isValidEmail(value)) {
        setErrors((prev) => ({
          ...prev,
          professionalEmail: "Please enter a valid email address.",
        }));
      } else {
        setErrors((prev) => {
          const { professionalEmail, ...rest } = prev;
          return rest;
        });
      }
    }
      ...
      <form className="space-y-4">
        ...
        <div className="form-group mb-4">
          ...
          {errors.professionalEmail && (
            <p className="text-red-500 text-sm mt-2">{errors.professionalEmail}</p>
          )}
        </div>
