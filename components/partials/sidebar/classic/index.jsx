...
import { generateNavBar, menusConfig } from "@/config/menus";
...
import { Cart, ClipBoard, DashBoard, Graph, Messages } from "@/components/svg";
...
const ClassicSidebar = ({ trans }) => {
  ...
  useEffect(()=>{
    if(user){
      const nav=generateNavBar(user.roles.permissions)
      setMenus(nav)
    }
  },[user])
