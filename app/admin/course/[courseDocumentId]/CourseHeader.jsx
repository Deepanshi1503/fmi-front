...
const CourseHeader = ({ course }) => {
  ...
  const makeEnrollment = async () => {
    try {
      ...
      const { data } = await axiosInstance({
        url: `/api/course-enrollments?filters[user][id][$eq]=${user?.id}&filters[courses][documentId][$eq]=${course?.documentId}`,
        method: 'get'
      });
      ...
      if (data.data.length > 0) {
        ...
        setMsg('Already enrolled')
      } else {
        //new enrollment with exisiting courses
        const { data: currentData } = await axiosInstance({
          url: `/api/course-enrollments?filters[user][id][$eq]=${user?.id}&populate=*`,
          method: 'get',
        });
        const existingIds = currentData?.data[0]?.courses?.map((q) => q?.id) || [];
        const updatedIds = Array.from(new Set([...existingIds, course?.id]));
        // console.log(currentData,"and ",existingIds, updatedIds)
        const method = existingIds.length > 0 ? 'put' : 'post';
        const url = existingIds.length > 0
          ? `/api/course-enrollments/${currentData.data[0]?.documentId}`
  ...
  const HeaderInfo = async()=>{
    try {
      const {data} = await axiosInstance({
        url:`/api/course-enrollments?filters[courses][documentId][$eq]=${course?.documentId}`
      });
      ...
      <CardContent className="border-b border-default-200">
        <div className="flex flex-col md:flex-row gap-6">
          <div className="flex-none">
            <div className="h-[400px] w-[500px] rounded-md">
              <Image
          ...
          <div className="flex-1">
            ...
            <div className="mt-0 flex flex-wrap items-center gap-6">
              ...
              <div className="border border-solid border-default-300 rounded-md py-6 px-6 min-w-fit lg:min-w-[280px] relative">
            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-6 lg:gap-6 mt-6">
              ...
              <div className="">
                ...
                >
                  {loading ? "enrolling..." : enrollmentStatus ? msg : "Enroll Now"}
                </Button>
              </div>
            </div>
